...
`gh-ruleset` enhances `gh` with a `ruleset` command group so repository administrators can manage Rulesets straight from the terminal. It wraps the official REST API and provides interactive flows to create, update, inspect, and delete rulesets — including fine-grained control over status checks and bypass actors.
...
```bash
gh extension install <your-user-or-org>/gh-ruleset
```
...
Uninstall at any time with `gh extension remove gh-ruleset`.
...
gh ruleset view 42
gh ruleset view 42 --json

# Create or update rulesets interactively
gh ruleset create
gh ruleset update 42

# Manage individual rules inside a ruleset
gh ruleset rule list 42
gh ruleset rule add 42
gh ruleset rule edit 42 1     # 1-based index
gh ruleset rule delete 42 2

# Discover recently observed status checks
gh ruleset checks --repo owner/repo
gh ruleset checks --pr 123              # include PR #123
gh ruleset checks --latest-pr           # latest open PR, or latest merged if none open
gh ruleset checks --no-default --ref <sha-or-branch>
```
...
Running `gh ruleset create` or `gh ruleset update` without `--file` launches an assistant that covers:
...
```bash
gh ruleset create --from-existing 42
gh ruleset update 42 --file path/to/ruleset.json
```
...
- `gh ruleset checks` aggregates contexts across all requested refs and prints their provenance (`default:main`, `pr#123`, etc.).
