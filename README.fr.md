...
```bash
gh extension install <owner>/gh-ruleset
```

En développement local, il suffit d’exécuter `./gh-ruleset …`. Une fois le repo cloné, vous pouvez tester l’extension avant publication avec :
...
gh ruleset checks --pr 42            # inclut la PR #42
gh ruleset checks --latest-pr        # ajoute la PR ouverte la plus récente (puis fusionnée sinon)
gh ruleset checks --no-default --ref 123abc456  # SHA précis sans la branche par défaut
```
...
- `--file ruleset.json` : charge un JSON existant (exporté via `gh ruleset view --json` par exemple).
- `--editor` après l’assistant : ouvre l’objet final dans votre éditeur avant de l’envoyer à l’API.
- `gh ruleset create --from-existing ID` : clone un ruleset avant de lancer l’assistant.
...
Référez-vous à la documentation GitHub pour l’exhaustivité des paramètres des règles : `gh ruleset view --json` fournit une base modifiable, et l’édition JSON libre permet d’utiliser toutes les fonctionnalités disponibles.
